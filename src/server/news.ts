...
function sanitizeArticles(articles: NewsArticle[]): NewsArticle[] {
  ...
  return deduped.map((article) => {
    ...
    return { ...article, id: `${article.id}-${stableSuffix}-${count}` };
  });
}
...
function normalizeExternalArticle(input: {
  ...
  if (isCodeLike(title) || isCodeLike(summary)) return null;
  if (title.length <= 10 || !link) return null;
...
async function fetchFromNewsApi(apiKey: string): Promise<ProviderResult> {
  ...
  try {
    ...
    if (data.status !== 'ok') {
      return { provider: 'newsapi', articles: [], ok: false, error: data.message ?? 'NewsAPI returned non-ok status' };
    }
...
async function fetchFromAlphaVantage(apiKey: string): Promise<ProviderResult> {
  ...
  try {
    ...
    if (data.Note || data.Information) {
      return { provider: 'alphavantage', articles: [], ok: false, error: data.Note ?? data.Information };
    }
...
async function fetchFromMarketaux(apiKey: string): Promise<ProviderResult> {
  ...
  try {
    ...
    if (data.error) return { provider: 'marketaux', articles: [], ok: false, error: data.error.message };
...
async function fetchFromTiingo(apiKey: string): Promise<ProviderResult> {
  ...
  try {
    const data = await fetchJson<TiingoArticle[]>('https://api.tiingo.com/tiingo/news', {
...
async function fetchFromConfiguredSource(source: FeedSource): Promise<ProviderResult> {
  ...
  if (provider === 'rss') return fetchRssSource(source.url, source.name);
  ...
  return { provider: source.id, articles: [], ok: false, error: 'Unsupported source provider mapping' };
}

function sourcesSignature(sources: FeedSource[]): string {
  return sources.map((s) => `${s.id}:${s.enabled}:${s.priority}:${s.url}`).sort().join('|');
}
...
export async function getAggregatedNews(): Promise<AggregatedNews> {
  ...
  if (
    ...
    return {
      ...
      usingFallback: newsCache.usingFallback,
      provider: newsCache.provider,
      fromCache: true,
      status: newsCache.usingFallback ? 'fallback' : 'live',
      warnings: [],
    };
  }
  ...
  const liveResults = providerResults.filter((r) => r.articles.length > 0);

  if (liveResults.length > 0) {
    const merged = liveResults.flatMap((r) => r.articles);
    const sanitized = sanitizeArticles(merged);
    const providerName = liveResults.map((r) => r.provider).join(',');
  ...
  const warnings = providerResults.map((r) =>
    !r.ok && r.error ? `${r.provider}: ${r.error}` : `${r.provider}: insufficient articles`,
  );

  return {
    articles: fallbackArticles,
    usingFallback: fallback.usingFallback,
