...
export function useSpeechReader(articles: NewsArticle[], filterKey?: string) {
  ...
  const togglePlayPause = useCallback(() => {
    if (typeof window === 'undefined' || !hasSpeechSupport(window)) return;
    if (state.isPlaying) {
      // Disable auto-advance synchronously before pause/cancel so Firefox's
      // immediate onend callback sees the updated ref and won't auto-advance.
      autoplayRef.current = false;
      isPlayingRef.current = false;
  ...
  const stopReading = useCallback(() => {
    if (typeof window === 'undefined' || !hasSpeechSupport(window)) return;
    // Disable auto-advance synchronously before cancel so Firefox's
